...
def prepare_comunicacoes(email: pd.DataFrame, im: pd.DataFrame, dim_lookup: pd.DataFrame) -> pd.DataFrame:
    ...
    im_df["Template_Acao_Grupo"] = im_df["Template_Acao"]
...
def apply_filters(interessados: pd.DataFrame, comunicacoes: pd.DataFrame):
    ...
    for item in email_group_order:
        if item in comunicacoes["Template_Acao_Grupo"].unique():
            template_options.append(item)

    im_templates = sorted(
        x
        for x in comunicacoes.loc[comunicacoes["Canal"].isin(["WhatsApp", "SMS", "Push"]), "Template_Acao_Grupo"]
        .dropna()
        .unique()
        .tolist()
        if x
    )
    raw_email_leftovers = sorted(
        x
        for x in comunicacoes.loc[comunicacoes["Canal"] == "Email", "Template_Acao_Grupo"]
        .dropna()
        .unique()
        .tolist()
        if x not in email_group_order and x
    )
    template_options.extend(im_templates)
    template_options.extend(raw_email_leftovers)
    ...
    if isinstance(date_range, tuple) and len(date_range) == 2:
        dt_ini, dt_fim = pd.Timestamp(date_range[0]), pd.Timestamp(date_range[1]) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)
    else:
...
def main():
    ...
    total_ucs_interessadas = interessados["NUM_UC"].nunique()

    st.title("BI Comunicação Plano Fixo")

    f_int, f_com, municipio = apply_filters(interessados, comunicacoes)

    # interested base always means unique UCs in df_interessados, statuses A/I/R/D/X
    ...
    with c5:
        metric_card("UCs contactadas", f"{contacted_ucs_filtered:,}".replace(",", "."),
                    "UCs únicas presentes nas comunicações após os filtros.")
    with c6:
        metric_card("% das UCs contactadas que demonstraram interesse",
                    f"{pct_contacted_that_interested:,.1f}%".replace(",", "X").replace(".", ",").replace("X", "."),
                    "UCs interessadas com contato dividido pelas UCs contactadas no filtro.")
    with c7:
    ...
    build_map(f_int if municipio == "Todos" else f_int.assign(MUNICIPIO=municipio))
