...
def load_data_cached():
    interessados = read_parquet_source(REQUIRED_FILES["interessados"])
    email = read_parquet_source(REQUIRED_FILES["email"])
    im = read_parquet_source(REQUIRED_FILES["im"])
    ...
    uc_dims_cols = [
        ...
        "com_bandeira", "rural", "lat", "lon", "IND_SITUACAO"
    ]
...
VALID_STATUS = {"A", "I", "R", "D", "X"}


def base_ucs_interessadas(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    if "IND_SITUACAO" in out.columns:
        out = out[out["IND_SITUACAO"].isin(VALID_STATUS)]
    if "NUM_UC" in out.columns:
        out = out.dropna(subset=["NUM_UC"]).drop_duplicates("NUM_UC")
    return out


def summarize_uc_coverage(filtered_interessados: pd.DataFrame, filtered_comms: pd.DataFrame):
    base_ucs = base_ucs_interessadas(filtered_interessados)
    total_ucs = base_ucs["NUM_UC"].nunique() if "NUM_UC" in base_ucs.columns else 0
    ucs_comunicadas = 0
    if total_ucs and "uc_join" in filtered_comms.columns:
        ucs_comunicadas = int(base_ucs["NUM_UC"].isin(filtered_comms["uc_join"].dropna().unique()).sum())
    pct = (ucs_comunicadas / total_ucs * 100) if total_ucs else 0.0
    return total_ucs, ucs_comunicadas, pct


def cumulative_interest(df):
    base = df.dropna(subset=["DTH_INTERESSE"]).copy() if "DTH_INTERESSE" in df.columns else pd.DataFrame()
    base = base_ucs_interessadas(base)
    if base.empty:
        return pd.DataFrame(columns=["Data", "UCs interessadas do dia", "Acumulado"])
    daily = (
        base.groupby(base["DTH_INTERESSE"].dt.date)["NUM_UC"]
        .nunique()
        .rename("UCs interessadas do dia")
        .reset_index()
        .rename(columns={"DTH_INTERESSE": "Data"})
    )
    daily["Data"] = pd.to_datetime(daily["Data"])
    all_days = pd.DataFrame({"Data": pd.date_range(daily["Data"].min(), daily["Data"].max(), freq="D")})
    daily = all_days.merge(daily, on="Data", how="left").fillna({"UCs interessadas do dia": 0})
    daily["UCs interessadas do dia"] = daily["UCs interessadas do dia"].astype(int)
    daily["Acumulado"] = daily["UCs interessadas do dia"].cumsum()
    return daily
...
def make_municipio_map(df_municipio):
    ...
    for _, row in valid.iterrows():
        popup = folium.Popup(
            ...
            UCs interessadas: {int(row['ucs_interessadas'])}<br>
        ...
        folium.CircleMarker(
            ...
            radius=max(6, min(18, np.sqrt(row["ucs_interessadas"]) + 4)),
            popup=popup,
...
try:
    ...
    st.error(str(e))
    st.info("Coloque os arquivos .parquet no repositório, na raiz ou na pasta data/.")
    st.stop()
...
with st.sidebar:
    ...
    municipios = st.multiselect(
        "Município",
        all_municipios,
        key="selected_municipios",
        help="Clique no mapa para preencher este filtro automaticamente, ou selecione manualmente aqui.",
    )
...
municipio_summary = (
    ...
    .agg(ucs_interessadas=("NUM_UC", "nunique"), lat=("lat", "median"), lon=("lon", "median"))
)
if not municipio_summary.empty:
    ...
    municipio_summary = pd.DataFrame(columns=["MUNICIPIO", "ucs_interessadas", "lat", "lon", "plano_top"])
...
with col_map:
    ...
    if fmap is not None:
        ...
        if last_obj:
            st.session_state["selected_municipios"] = [last_obj]
            st.rerun()
    else:
...
with col_kpi:
    st.subheader("Resumo")
    total_ucs_interessadas, ucs_comunicadas, pct_ucs_comunicadas = summarize_uc_coverage(filtered_interessados, filtered_comms)
    ...
    k1, k2 = st.columns(2)
    k1.metric("UCs interessadas", f"{total_ucs_interessadas:,}".replace(",", "."))
    k2.metric("UCs interessadas com comunicação", f"{ucs_comunicadas:,}".replace(",", "."))
    k3, k4 = st.columns(2)
    k3.metric("% UCs interessadas com comunicação", f"{pct_ucs_comunicadas:.1f}%")
    k4.metric("Comunicações filtradas", f"{total_comms:,}".replace(",", "."))
    ...
    if not filtered_interessados.empty and "prazo_plano" in filtered_interessados.columns:
        ...
        prazo_mix = base_ucs_interessadas(filtered_interessados)["prazo_plano"].fillna("Sem classificação").value_counts(dropna=False)
...
with row1_col1:
    ...
    if not cum_int.empty:
        fig = px.line(cum_int, x="Data", y="Acumulado", markers=True, hover_data=["UCs interessadas do dia"])
        fig.update_layout(margin=dict(l=10, r=10, t=10, b=10), yaxis_title="Acumulado")
        st.plotly_chart(fig, width="stretch")
    else:
        st.info("Sem UCs interessadas para os filtros atuais.")
with row1_col2:
...
with row2_col1:
    ...
    plano_counts = base_ucs_interessadas(filtered_interessados).get("plano_nome", pd.Series(dtype=str)).fillna("Sem classificação").value_counts().rename_axis("Plano").reset_index(name="Qtd")
...
with row3_col1:
    ...
    muni_counts = base_ucs_interessadas(filtered_interessados).get("MUNICIPIO", pd.Series(dtype=str)).fillna("Sem município").value_counts().head(20).rename_axis("Município").reset_index(name="Qtd")
...
with tab1:
    ...
    export_int = base_ucs_interessadas(filtered_interessados)[cols].copy()
