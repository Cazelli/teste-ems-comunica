...
# Custos gerais informados pela equipe de comunicação.
# Observação: Push e SMS podem divergir dos totais existentes nos parquet.
# Por isso, mantemos os totais oficiais abaixo para o totalizador geral
# e calculamos o custo por mensagem com UC conhecida usando as mensagens
# que aparecem no app com NUM_UC identificado.
MESSAGING_COSTS = {
    "Push": {"envios_reportados": 169_100, "custo_total": 5_073.00, "custo_por_envio_reportado": 0.03},
    "SMS": {"envios_reportados": 526_069, "custo_total": 42_085.52, "custo_por_envio_reportado": 0.08},
    "WhatsApp": {"envios_reportados": 282_428, "custo_total": 118_083.80, "custo_por_envio_reportado": 0.42},
}
MEDIA_COSTS = {
    "Meta": {"cliques": 133_797, "investimento": 141_997.51, "custo_por_clique": 1.06},
    "Google": {"cliques": 6_688, "investimento": 24_779.80, "custo_por_clique": 3.71},
    "OneStation": {"cliques": 23_368, "investimento": 40_000.00, "custo_por_clique": 1.71},
    "TV aberta": {"cliques": None, "investimento": 20_802.88, "custo_por_clique": None},
    "Rádio": {"cliques": None, "investimento": 17_140.32, "custo_por_clique": None},
    "Portais": {"cliques": None, "investimento": 7_849.00, "custo_por_clique": None},
}
TOTAL_MESSAGING_REPORTED_SENDS = 977_597
TOTAL_MESSAGING_COST = 165_242.32
TOTAL_MEDIA_COST = 252_569.51
TOTAL_GENERAL_COST = 417_811.83
COSTED_MESSAGE_CHANNELS = list(MESSAGING_COSTS.keys())
...
def build_cost_context(comunicacoes: pd.DataFrame) -> dict:
    """Build cost rates based on messages with known NUM_UC in the app."""
    known = comunicacoes[
    ...
    rates_known = {}
    messages_without_user_report = 0
    for channel, info in MESSAGING_COSTS.items():
        known_messages = float(known_messages_by_channel.get(channel, 0))
        reported_messages = float(info["envios_reportados"])
        rates_known[channel] = info["custo_total"] / known_messages if known_messages > 0 else 0
...
def estimate_messaging_cost(df: pd.DataFrame, cost_context: dict) -> float:
    if df.empty:
        return 0.0
    by_channel = (
        df[df["Canal"].isin(COSTED_MESSAGE_CHANNELS)]
...
def render_cost_report(cost_context: dict):
    ...
    render_metric_block(
        "",
        [
            ("Custo total geral", format_currency(TOTAL_GENERAL_COST), "Mensageria + mídia, conforme tabela informada."),
            ("Custo de mensageria", format_currency(TOTAL_MESSAGING_COST), "Push + SMS + WhatsApp, conforme tabela informada."),
            ("Investimento em mídia", format_currency(TOTAL_MEDIA_COST), "Meta + Google + OneStation + TV aberta + Rádio + Portais."),
            ("Envios reportados", format_int(TOTAL_MESSAGING_REPORTED_SENDS), "Total oficial de envios Push + SMS + WhatsApp da tabela."),
            ("Mensagens com UC conhecida", format_int(cost_context["known_costed_messages"]), "Mensagens Push + SMS + WhatsApp presentes no app com NUM_UC identificado."),
            ("Mensagens sem UC no report", format_int(cost_context["messages_without_user_report"]), "Diferença positiva entre o total reportado na tabela e as mensagens com UC conhecida no app."),
    ...
    for channel, info in MESSAGING_COSTS.items():
        ...
        cost_rows.append(
            {
                "Canal": channel,
                "Envios reportados": info["envios_reportados"],
                "Mensagens com UC conhecida": known_msgs,
                "Mensagens sem UC no report": max(float(info["envios_reportados"]) - known_msgs, 0),
                "Custo total": info["custo_total"],
                "Custo/envio reportado": info["custo_por_envio_reportado"],
    ...
    with st.expander("Detalhamento de investimento em mídia"):
        media_rows = []
        for source, info in MEDIA_COSTS.items():
            media_rows.append({"Mídia": source, **info})
        st.dataframe(pd.DataFrame(media_rows), width="stretch", height=240)
...
def render_overview_page(interessados: pd.DataFrame, comunicacoes: pd.DataFrame):
    cost_context = build_cost_context(comunicacoes)
...
def render_comparison_page(interessados: pd.DataFrame, comunicacoes: pd.DataFrame):
    cost_context = build_cost_context(comunicacoes)
...
def main():
    ...
    if page == "Visão Geral":
        render_overview_page(interessados, comunicacoes)
    else:
        render_comparison_page(interessados, comunicacoes)
